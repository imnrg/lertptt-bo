...
export default function ProductsPage() {
	...
	const fetchProducts = async () => {
		try {
			// ดึงเฉพาะผลิตภัณฑ์ที่ไม่ใช่เชื้อเพลิง (fuelTypeId เป็น null)
			const response = await fetch('/api/products?excludeFuel=true')
			if (response.ok) {
		...
		<div className="space-y-6">
			...
			<div className="flex justify-between items-center">
				<div>
					...
					<p className="mt-2 text-sm text-gray-700">จัดการสินค้าและตรวจสอบสต็อกสินค้า</p>
				</div>
				<Button asChild>
